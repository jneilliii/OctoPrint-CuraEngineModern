...
class Profile(object):
	...
	def get(self, key):
		if key in ("machine_width", "machine_depth", "machine_center_is_zero"):
			if key == "machine_width":
				...
				return self._printer_profile["volume"]["formFactor"] == "circular" or self._printer_profile["volume"][
					"origin"] == "center"
			else:
				return None
	...
	def replaceTagMatch(self, m):
		...
		elif pre == 'F' and tag in ['print_speed', 'retraction_speed', 'travel_speed', 'bottom_layer_speed',
									'cool_min_feedrate']:
	...
	def get_gcode(self, key, extruder_count=1):
		...
		return unicode(prefix + re.sub("(.)\{([^\}]*)\}", self.replaceTagMatch,
	...
	def convert_to_engine(self, used_extruders=1):
		...
		settings = {
			...
			"initialLayerThickness": self.get_microns("bottom_thickness") if self.get_float(
				"bottom_thickness") > 0.0 else self.get_microns("layer_height"),
			...
			"infillSpeed": self.get_int("infill_speed") if self.get_int("infill_speed") > 0 else self.get_int(
				"print_speed"),
			"inset0Speed": self.get_int("outer_shell_speed") if self.get_int("outer_shell_speed") > 0 else self.get_int(
				"print_speed"),
			"insetXSpeed": self.get_int("inner_shell_speed") if self.get_int("inner_shell_speed") > 0 else self.get_int(
				"print_speed"),
			"skinSpeed": self.get_int("solidarea_speed") if self.get_int("solidarea_speed") > 0 > 0 else self.get_int(
				"print_speed"),
			...
			"supportAngle": int(-1) if self.get("support") == SupportLocationTypes.NONE else self.get_int(
				"support_angle"),
			"supportEverywhere": int(1) if self.get("support") == SupportLocationTypes.EVERYWHERE else int(0),
			"supportLineDistance": int(100 * edge_width * 1000 / self.get_float("support_fill_rate")) if self.get_float(
				"support_fill_rate") > 0 else -1,
			"supportXYDistance": int(1000 * self.get_float("support_xy_distance")),
			"supportZDistance": int(1000 * self.get_float("support_z_distance")),
			"supportExtruder": 0 if self.get("support_dual_extrusion") == SupportDualTypes.FIRST else (1 if self.get(
				"support_dual_extrusion") == SupportDualTypes.SECOND and minimal_extruder_count > 1 else -1),
			...
			"enableCombing": 1 if self.get("retraction_combing") == RetractionCombingTypes.ALL else (
				2 if self.get("retraction_combing") == RetractionCombingTypes.NO_SKIN else 0),
		...
		for extruder in range(1, extruder_count):
			for axis in ("x", "y"):
				settings["extruderOffset[{extruder}].{axis}".format(extruder=extruder, axis=axis.upper())] = self.get(
					"extruder_offset_{axis}{extruder}".format(extruder=extruder, axis=axis.lower()))

		fanFullHeight = self.get_microns("fan_full_height")
		settings["fanFullOnLayerNr"] = (fanFullHeight - settings["initialLayerThickness"] - 1) // settings[
			"layerThickness"] + 1
		...
		if self.get_boolean("wipe_tower") and extruder_count > 1:
			...
			settings["wipeTowerSize"] = int(
				math.sqrt(self.get_float("wipe_tower_volume") * 1000 * 1000 * 1000 / settings["layerThickness"]))
