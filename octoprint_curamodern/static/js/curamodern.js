$(function() {
    function CuraEngineModernViewModel(parameters) {
        var self = this;
        ...
        self.uploadDialog = $("#settings_plugin_curamodern_import");
        self.uploadElement = $("#settings-curamodern-import");
        ...
        self.removeProfile = function(data) {
            ...
            OctoPrint.slicing.deleteProfileForSlicer("curamodern", data.key, {url: data.resource()})
                .done(function() {
                    self.requestData();
                    self.slicingViewModel.requestData();
                });
        ...
        self.makeProfileDefault = function(data) {
            ...
            OctoPrint.slicing.updateProfileForSlicer("curamodern", data.key, {default: true}, {url: data.resource()})
                .done(function() {
                    self.requestData();
                });
        };
        ...
        self.testEnginePath = function() {
            OctoPrint.util.testExecutable(self.settings.plugins.curamodern.cura_engine())
        ...
        self.onAllBound = function() {
            self.uploadDialog.on("hidden", function(event) {
                if (event.target.id === "settings_plugin_curamodern_import") {
                    self.clearUpload();
                }
            });
        };
        ...
        self.onSlicingData = function(data) {
            if (data && data.hasOwnProperty("curamodern") && data.curamodern.hasOwnProperty("profiles")) {
                self.fromResponse(data.curamodern.profiles);
            }
        };
        ...
        self.onWizardDetails = function(response) {
            if (!response.hasOwnProperty("curamodern") || !response.curamodern.required) return;

            if (response.curamodern.details.hasOwnProperty("engine")) {
                self.unconfiguredCuraEngine(!response.curamodern.details.engine);
            }
            if (response.curamodern.details.hasOwnProperty("profile")) {
                self.unconfiguredSlicingProfile(!response.curamodern.details.profile);
            }
        };
    ...
    OCTOPRINT_VIEWMODELS.push({
        construct: CuraEngineModernViewModel,
        dependencies: ["loginStateViewModel", "settingsViewModel", "slicingViewModel"],
        elements: ["#settings_plugin_curamodern", "#wizard_plugin_curamodern"]
    });
});
