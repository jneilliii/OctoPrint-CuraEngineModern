...
class CuraEngineModernPlugin(octoprint.plugin.SlicerPlugin,
	...
	def __init__(self):
		self._logger = logging.getLogger("octoprint.plugins.curamodern")
		self._cura_logger = logging.getLogger("octoprint.plugins.curamodern.engine")
	...
	def initialize(self):
		# migrate settings
		old_config = self._settings.global_get(["plugins", "cura"], asdict=True)
		if old_config:
			self._logger.info("Migrating settings from cura to curamodern...")
			self._settings.set(["cura_engine"], old_config.get("cura_engine", None))
			self._settings.set(["default_profile"], old_config.get("default_profile", None))
			self._settings.set_boolean(["debug_logging"], old_config.get("debug_logging", False))
			self._settings.global_set(["plugins", "cura"], None)
			self._settings.save()

		# migrate profiles
		old_path = os.path.join(self._slicing_manager._profile_path, "cura")
		new_path = os.path.join(self._slicing_manager._profile_path, "curamodern")
		if os.path.exists(old_path) and not os.path.exists(new_path):
			self._logger.info("Migrating slicing profiles from cura to curamodern...")
			import shutil
			try:
				shutil.move(old_path, new_path)
			except Exception:
				self._logger.exception("Error while migrating slicing profiles from cura to curamodern")

		# migrate selected default profile
		old_default = self._settings.global_get(["slicing", "defaultProfiles", "cura"])
		if old_default:
			self._logger.info("Migrating default slicing profile from cura to curamodern...")
			self._settings.global_set(["slicing", "defaultProfiles", "curamodern"], old_default)
			self._settings.global_set(["slicing", "defaultProfiles", "cura"], None)
			self._settings.save()
	...
	def _is_profile_available(self):
		return bool(self._slicing_manager.all_profiles("curamodern", require_configured=False))

	##~~ TemplatePlugin API

	def get_template_vars(self):
		return dict(
			homepage="https://github.com/jneilliii/OctoPrint-CuraEngineModern/"
		)
	...
	def on_startup(self, host, port):
		...
		cura_logging_handler = CleaningTimedRotatingFileHandler(
			self._settings.get_plugin_logfile_path(postfix="engine"), when="D", backupCount=3)
	...
	def import_cura_profile(self):
		...
		if input_upload_name in flask.request.values and input_upload_path in flask.request.values:
			...
			try:
				filepath = os.path.join(self.get_plugin_data_folder(), filename)
				upload = octoprint.filemanager.util.DiskFileWrapper(flask.request.values[input_upload_name],
																	flask.request.values[input_upload_path]).save(
					filepath)
			except Exception as e:
				self._logger.exception(u"Error while converting the imported profile")
				return flask.make_response(
					u"Something went wrong while converting imported profile: {message}".format(message=str(e)), 500)
		...
		#
		# try:
		# 	self._slicing_manager.save_profile("curamodern",
		# 	                                   profile_name,
		# 	                                   profile_dict,
		...
		# 	return flask.make_response(u"A profile named {profile_name} already exists for slicer curamodern".format(**locals()), 409)
		#
		# if profile_make_default:
		# 	try:
		# 		self._slicing_manager.set_default_profile("curamodern", profile_name)
		# 	except octoprint.slicing.UnknownProfile:
		# 		self._logger.warn(u"Profile {profile_name} could not be set as default, aborting".format(**locals()))
		# 		return flask.make_response(u"The profile {profile_name} for slicer curamodern could not be set as default".format(**locals()), 500)
		#
		# result = dict(
		# 	resource=flask.url_for("api.slicingGetSlicerProfile", slicer="curamodern", name=profile_name, _external=True),
		# 	name=profile_name,
		# 	displayName=profile_display_name,
		# 	description=profile_description
		# )
		r = flask.make_response(flask.jsonify(dict(file=flask.request.values[input_upload_name])), 201)
	...
	def get_assets(self):
		return {
			"js": ["js/curamodern.js"],
			"less": ["less/curamodern.less"],
			"css": ["css/curamodern.css"]
		}
							...
							try:
								...
								if slicing_profile.get_float("filament_diameter") is not None:
									if slicing_profile.get(
											"gcode_flavor") == GcodeFlavors.ULTIGCODE or slicing_profile.get(
											"gcode_flavor") == GcodeFlavors.REPRAP_VOLUME:
										analysis["filament"][tool_key] = _get_usage_from_volume(filament,
																								slicing_profile.get_float(
																									"filament_diameter"))
									else:
										analysis["filament"][tool_key] = _get_usage_from_length(filament,
																								slicing_profile.get_float(
																									"filament_diameter"))
	...
	def get_update_information(self):
		return dict(
			curamodern=dict(
				displayName="CuraEngine Modern",
				displayVersion=self._plugin_version,

				# version check: github repository
				type="github_release",
				user="jneilliii",
				repo="OctoPrint-CuraEngineModern",
				current=self._plugin_version,

				# update method: pip
				pip="https://github.com/jneilliii/OctoPrint-CuraEngineModern/archive/{target_version}.zip"
			)
		)
...
__plugin_name__ = "CuraEngine Modern"
__plugin_pythoncompat__ = ">=2.7,<4"


def __plugin_load__():
	global __plugin_implementation__
	__plugin_implementation__ = CuraEngineModernPlugin()
